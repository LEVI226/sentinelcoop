...
**a. Les moteurs du marché ne reconnaissent pas les noms ouest-africains.** Soundex et Metaphone sont conçus pour l'anglais. Ils échouent sur des variations banales dans nos états civils : `Ouédraogo` / `Wedraogo`, `Diallo` / `Jallo`, `Cissé` / `Sisse`, `Mohamed` / `Muhammad` / `Mahamadou`, `El Hadj` / `Alhaji`, et sur l'inversion nom/prénom entre la pièce d'identité et la saisie au guichet. D'où deux défaillances symétriques et également graves : le **faux négatif** — une personne listée entre en relation d'affaires sans être détectée — et le **faux positif de masse**, qui noie l'agent de conformité sous des alertes ingérables et conduit, en pratique, à ce que plus personne ne les traite.

**b. La connectivité ne peut pas conditionner le contrôle.** Une part significative des points de service opère avec une connexion intermittente. Toute solution exigeant un appel réseau synchrone au guichet est soit bloquante pour l'activité, soit contournée.

**c. Le coût de licence est prohibitif.** Les plateformes du marché sont tarifées pour des banques traitant des millions de requêtes. Rapporté au portefeuille d'une coopérative, l'institution reste de fait non équipée.

**d. Le « sans délai » n'est pas mesuré, donc pas prouvable.** Dans un dispositif à mise à jour manuelle, personne ne peut répondre à la question : *de quand date la liste que nous utilisons en ce moment ?* L'article 198 punit pourtant de 100 000 à 1 500 000 FCFA le manquement aux obligations de gel résultant d'une simple **imprudence ou négligence**. Ce qui n'est pas horodaté ne se prouve pas devant un contrôleur.
...
**M1 — Référentiel et synchronisation différentielle.** Agrégation des listes publiques et gratuites (Conseil de sécurité de l'ONU, Union européenne, OFAC/SDN), des listes nationales notifiées au titre de l'article 124 et des PPE internes. Le poste ne télécharge que les entrées ajoutées, modifiées ou radiées depuis son dernier état connu, sous forme d'un correctif signé de quelques kilo-octets, reprenant automatiquement après coupure. Un indicateur permanent affiche l'âge exact du référentiel — *« liste ONU synchronisée il y a 4 minutes »* — et journalise chaque correctif appliqué. C'est cet horodatage qui rend le « sans délai » **démontrable**.

**M2 — Moteur de correspondance phonétique ouest-africaine.** Cœur technique, en trois couches : un **encodage phonétique** construit sur les règles de translittération propres à l'espace UEMOA (digraphes `ou`/`w`/`u`, diacritiques, équivalences `c`/`k`/`q` et `s`/`c`/`ç`, titres honorifiques) ; une **distance de chaînes hybride** (Jaro-Winkler et Damerau-Levenshtein, jeton par jeton) pour les fautes de frappe et inversions ; une **pondération par rareté du jeton** — dans un portefeuille burkinabè, `Ouédraogo` porte très peu d'information discriminante là où un prénom rare en porte beaucoup. C'est le levier principal de réduction des faux positifs. Le score est confronté à un **double seuil paramétrable** : au-delà du seuil haut, l'alerte est **bloquante** et l'opération suspendue (art. 91) ; entre les deux seuils, elle est **informative** et alimente une file de revue. Chaque décision — levée, confirmation, escalade — est motivée et horodatée dans une piste d'audit inaltérable.

**M3 — Profilage client et consolidation multi-comptes.** Fiche de risque par client (catégorie habituel/occasionnel, qualité de PPE, secteur, zone, historique d'alertes — art. 13 b). Calcul continu du **solde global consolidé** de l'ensemble des comptes d'un même client, toutes agences confondues, et **recensement des opérations** sur période glissante, qui détecte le franchissement de seuil par accumulation d'opérations individuellement anodines.

**M4 — Surveillance comportementale.** Le suivi des mouvements alimente un graphe de relations entre comptes : détection de **fractionnement**, de **comptes rebond**, de cycles de transferts entre comptes liés, de ruptures de tendance par rapport à l'historique propre du client.

**M5 — Production automatique des actes de conformité.** La loi n'impose pas seulement de détecter : elle impose de **produire des actes**, dans des formes et des délais précis — c'est le maillon que les dispositifs manuels perdent en premier. Le **rapport confidentiel écrit** de l'article 21 et le **projet de déclaration de soupçon** de l'article 60 sont pré-remplis à partir des données déjà détenues, l'agent n'ayant qu'à porter son analyse. La **réévaluation triennale des PPE** (art. 29) est déclenchée automatiquement.
...
**3.1 — L'adaptation phonétique à l'onomastique ouest-africaine.** À notre connaissance, aucun moteur de filtrage de sanctions ne dispose d'un encodage calibré sur les systèmes de noms de l'espace UEMOA. **Nous ne l'affirmons pas : nous l'avons mesuré.** Un prototype du moteur est déjà opérationnel et filtre la liste consolidée réelle du Conseil de sécurité des Nations unies (1 011 entrées, 2 767 alias), sur un jeu de 99 couples de variantes ouest-africaines attestées :
...
**3.2 — Trois autres partis pris.** *Le hors-ligne d'abord comme choix de conformité* : un contrôle qui s'interrompt quand le réseau tombe est un contrôle qui n'existe pas ; nous inversons l'hypothèse du marché, le réseau devient l'exception et non la condition. *La preuve du « sans délai »* : transformer l'obligation qualitative de l'article 89 en un indicateur horodaté et journalisé déplace la question du contrôleur de « avez-vous une procédure ? » à « voici la trace ». *De la détection à la production des actes* : les outils du marché s'arrêtent à l'alerte, là où la loi exige des livrables formalisés.

**Ce que nous ne revendiquons pas.** Les briques élémentaires — correspondance approximative, détection de fractionnement — sont des techniques établies. Notre contribution porte sur leur calibrage à un contexte pour lequel elles n'ont jamais été réglées, et sur leur assemblage dans une architecture soutenable pour une coopérative.
...
Un ingénieur en informatique et intelligence artificielle, trois profils réseaux et télécommunications. Quatre axes distincts, chacun répondant à une contrainte nommée du défi.

**OUEDRAOGO Yannick U. L.** *(chef d'équipe, Master en informatique et IA)* — **moteur de correspondance et intégration au système d'information.** A travaillé chez un prestataire spécialisé dans le déploiement de **Core Banking System**, où il a développé un service d'**alertes bancaires par SMS destiné aux microfinances** : lire les événements de transaction dans la base du core banking et en déclencher une notification. Il en retire une familiarité concrète, acquise en production, avec la façon dont un système bancaire stocke comptes et opérations — ce dont dépend M3. Ancien du Pool WURI / ANSSI-BF.

**KOALA Wendpanga Gédéon** — **architecture hors-ligne et synchronisation.** Mikrotik et CCNA niveau avancé, expérience de terrain en configuration réseau, machine learning et Python. Le TDR impose une solution adaptée à une « connectivité limitée » : c'est un problème de réseau avant d'être un problème d'application. Lauréat de deux hackathons nationaux.

**MAMOUDOU Ayouba** — **sécurité du système d'information et piste d'audit.** Hacking éthique et analyse des risques, administration LAN/WAN/VPN, cloud et DevOps. Une piste d'audit modifiable ne vaut rien devant un contrôleur : l'article 35 de la loi 001-2025 et l'article 23 de la loi 046-2024 sont des exigences de sécurité, pas de fonctionnalité.

**GUEL Fabrice** — **restitution, expérience utilisateur au guichet et déploiement.** CCNA, fibre optique FTTX/GPON. A exercé une fonction de relation client de terrain chez un opérateur télécom : vulgariser une notion technique auprès d'un usager non spécialiste est exactement ce que demande une interface de revue d'alertes destinée à un agent de guichet.
...
**Sources.** Loi n° 046-2024/ALT du 30 décembre 2024 relative à la LBC/FT/FP (transposant la Loi uniforme UMOA de mars 2023) — Loi n° 001-2025/ALT du 13 février 2025 portant réglementation de la microfinance — CENTIF / CNCA-LBC/FT, *Évaluation Nationale des Risques BC/FT du Burkina Faso*, décembre 2025 — BCEAO, *Rapport sur la politique monétaire dans l'UMOA*, juin 2026.
